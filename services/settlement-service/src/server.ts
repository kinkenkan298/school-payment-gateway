...
import { startSettlementScheduler, stopSettlementScheduler } from './scheduler/settlement.scheduler';
...
const start = async () => {
  ...
  const server = app.listen(env.PORT, () => {
    logger.info({ port: env.PORT, env: env.NODE_ENV }, 'Settlement Service started');
  });
